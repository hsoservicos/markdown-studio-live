...
- **Ataque A1→A2→M1→M2 implementado e validado**: single-flight do mermaid (`renderMermaidDiagramsIn` + `renderInFlight`), reset durável (`src/ui/editorActions.js` → `removeItem(last_state)`), export PDF suspende o agendamento do mermaid durante a captura (`pauseMermaidScheduling`/`resumeMermaidScheduling`) e `scheduleConvertAndRender` (debounce 80ms) na digitação. **86 testes verdes** (+10: `mermaid.test.js`, `editorActions.test.js`, asserções em `exportPdf.test.js`); e2e real revalidado: `pdf_probe.js` (%PDF-, 500KB) e `reset_probe.js` (last_state null pós-reset, reload limpo) em Chrome headless.
